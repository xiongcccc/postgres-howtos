...
test('full text, empty input, no result and filters',()=>{
  ...
  assert.deepEqual(find('',{type:'conference'}),[99,100,101,102]);
...
test('series and source are independent and preserve old routes',()=>{
  assert.deepEqual(find('',{series:'interpretation'}),[101,102]);
  assert.deepEqual(find('',{series:'interpretation',topic:'ops'}),[102]);
...
test('generated internal article links exist and titles come from metadata',()=>{
  const files=['README.md','_sidebar.md','docs/topics.md','docs/paths.md','docs/conferences.md','docs/series.md','docs/101.md','docs/102.md'];
