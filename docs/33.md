...
> 原作者：Nikolay Samokhvalov · [英文原文](https://gitlab.com/postgres-ai/docs/-/blob/80536bb97f28d1007ad09627db434c2b7016da1f/docs/postgres-howtos/schema-design/ddl-operations/how-to-redefine-a-PK-without-downtime.md) · 中文翻译与校订：PostgreSQL学徒
...
**2025 年更新：**从 PostgreSQL 18 开始，`NOT NULL` 约束也支持 `NOT VALID`，可以将约束定义与存量数据的校验分开执行（[上游提交](https://git.postgresql.org/gitweb/?p=postgresql.git;a=commitdiff;h=a379061a2)）。上面的 `CHECK` 过渡方案仍适用于 PostgreSQL 12–17。
...
4. 基于现有的唯一索引和已验证的 `CHECK` 约束定义主键（隐式建立 `NOT NULL` 约束，跳过全表扫描）✂️：
