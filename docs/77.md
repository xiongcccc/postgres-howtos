...
> 原作者：Nikolay Samokhvalov · [英文原文](https://gitlab.com/postgres-ai/docs/-/blob/80536bb97f28d1007ad09627db434c2b7016da1f/docs/postgres-howtos/advanced-topics/replication/zero-downtime-major-upgrade.md) · 中文翻译与校订：xiongcc

2024 年 1 月前后，Hacker News 上讨论过 knock.app 的一篇[零停机升级 Postgres](https://news.ycombinator.com/item?id=38616181)文章。

GitLab 的 [Alexander Sosna](https://twitter.com/xxorde) 也在[演讲](https://www.postgresql.eu/events/pgconfeu2023/schedule/session/4791-how-we-execute-postgresql-major-upgrades-at-gitlab-with-zero-downtime/)中介绍过如何为高负载的大型集群实施零停机升级，值得参考。
...
一种常规方法是通过 `initdb` 创建新集群，再建立逻辑订阅，并设置 `copy_data = true`，将现有数据复制到新集群。然而，初始数据复制可能耗时很长，并对高负载系统造成较大压力。
...
首先，切换只读 (RO) 流量是有意义的。如果应用程序允许，首先将部分只读流量重定向到新的副本是有意义的。这需要在负载均衡代码中实现高级的复制滞后检测（参照：[Day 17: How to determine the replication lag - Hybrid case: logical & physical](/docs/17.md))。
...
上述流程只说明整体思路，实施时仍有许多细节需要处理。进一步的说明可参阅[演讲页面中的幻灯片和视频](https://www.postgresql.eu/events/pgconfeu2023/schedule/session/4791-how-we-execute-postgresql-major-upgrades-at-gitlab-with-zero-downtime/)。

## 我见

参照之前的文章 [How to convert a physical replica to logical](/docs/57.md?id=how-to-convert-a-physical-replica-to-logical)，另外推荐去阅读一下这篇 PDF：https://www.postgresql.eu/events/pgconfeu2023/schedule/session/4791-how-we-execute-postgresql-major-upgrades-at-gitlab-with-zero-downtime/，针对 logical replication + pg_upgrade 的升级方式进行了优化。
