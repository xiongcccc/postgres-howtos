...
> 原作者：Nikolay Samokhvalov · [英文原文](https://gitlab.com/postgres-ai/docs/-/blob/80536bb97f28d1007ad09627db434c2b7016da1f/docs/postgres-howtos/performance-optimization/indexing/how-to-find-unused-indexes.md) · 中文翻译与校订：PostgreSQL学徒
...
> 译者校订：上游基础查询直接读取 `indisunique`，但该字段属于 `pg_index`，不在 `pg_stat_user_indexes` 中，因此需要按 `indexrelid` 关联。查询结果只是待核查的候选索引，不能直接作为删除清单。
