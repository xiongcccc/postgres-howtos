...
> 原作者：Nikolay Samokhvalov · [英文原文](https://gitlab.com/postgres-ai/docs/-/blob/80536bb97f28d1007ad09627db434c2b7016da1f/docs/postgres-howtos/advanced-topics/misc/tuple-sparsity.md) · 中文翻译与校订：PostgreSQL学徒
...
11 次缓冲区命中，对应 88 KiB 的缓冲区访问量。下面的查询可以计算这 8 行数据的大小，这是上游示例中的结果：
...
也就是说，为返回 317 字节的数据，执行器处理了约 88 KiB 的缓冲区内容。这里使用的是 `Index Scan`，部分访问来自索引，部分来自堆表。这是缓冲区访问量，不应直接理解为实际磁盘读取量。
