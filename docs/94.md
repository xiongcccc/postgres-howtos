...
> 原作者：Nikolay Samokhvalov · [英文原文](https://gitlab.com/postgres-ai/docs/-/blob/80536bb97f28d1007ad09627db434c2b7016da1f/docs/postgres-howtos/database-administration/maintenance/how-to-run-analyze.md) · 中文翻译与校订：PostgreSQL学徒
...
下面的示例会在所有数据库上运行 ANALYZE（`--all`；某些托管服务可能限制对部分数据库的访问），使用 `$JOBS` 指定并发数，并为每条语句设置 2 小时的超时（此处省略 `-h`、`-U` 等连接参数）：
...
这个脚本会把执行的命令带上时间戳打印并记录日志（也可以使用 `moreutils` 提供的 `ts` 替代 `while` 循环）。

> 译者校订：`statement_timeout=2h` 限制的是单条语句，不是整个 `vacuumdb` 进程的总运行时间。多个数据库或多张表依次处理时，总耗时仍可能超过 2 小时。
