...
> 原作者：Nikolay Samokhvalov · [英文原文](https://gitlab.com/postgres-ai/docs/-/blob/80536bb97f28d1007ad09627db434c2b7016da1f/docs/postgres-howtos/database-administration/maintenance/how-to-enable-data-checksums-without-downtime.md) · 中文翻译与校订：PostgreSQL学徒

## 数据校验和，基础知识

PostgreSQL 支持[数据校验和](https://www.postgresql.org/docs/current/checksums.html)，可用于检测某些类型的数据损坏。校验和不能阻止损坏发生，也不能自动修复损坏。
...
3. 重新启动备库并让其完全追上主库。
4. 对所有其他备库重复步骤 1-3。
5. 执行主备切换。为缩短停机时间，建议在切换前显式执行 `CHECKPOINT`；如果使用 PgBouncer，可结合 `PAUSE/RESUME` 控制客户端流量。
