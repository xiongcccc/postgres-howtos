...
> 原作者：Nikolay Samokhvalov · [英文原文](https://gitlab.com/postgres-ai/docs/-/blob/80536bb97f28d1007ad09627db434c2b7016da1f/docs/postgres-howtos/advanced-topics/misc/how-to-help-others.md) · 中文翻译与校订：PostgreSQL学徒
...
1. 多会话实验，成熟的基准测试，例如使用 `pgbench`、`JMeter`、`sysbench`、`pgreplay-go` 等工具进行的基准测试。
   ...
   关于此类实验的更多信息：详见 [Day 13: How to benchmark](#/docs/13)。

2. 单会话实验，使用单个会话 (有时两个) 测试一个或一系列 SQL 查询，以检查查询语法、研究单个查询行为、优化特定查询等。

   这些实验可以在共享环境中进行，使用较弱的机器也可以。但是，要研究查询性能，你需要使用相同的 PostgreSQL 版本、相同或类似的数据库，以及匹配的规划器设置 (如何做到这一点：参照 [Day 56: How to make the non-production Postgres planner behave like in production](#/docs/56))。需要注意的是，计时指标可能与生产环境相比显著偏差，在查询优化过程中主要应关注执行计划和数据量 (`EXPLAIN` 中的实际行数、`BUFFERS` 选项提供的缓冲区操作计数)。
