...
> 原作者：Nikolay Samokhvalov · [英文原文](https://gitlab.com/postgres-ai/docs/-/blob/80536bb97f28d1007ad09627db434c2b7016da1f/docs/postgres-howtos/database-administration/backup-recovery/how-to-speed-up-bulk-load.md) · 中文翻译与校订：PostgreSQL学徒
...
如果数据加载到一个新表中，可以在数据加载完成后再创建索引。如果是加载到一个现有表中，[避免过多的索引。](#/docs/18)
...
- 分区表和多进程加载多个分区。可以使用多个工作进程并行加载 ([Day 20: pg_restore tips](#/docs/20))。
- 非分区表和大块数据加载。这需要事先准备大块数据 — 可以将 CSV 文件拆分成多个部分，或使用多个同步的 `REPEATABLE READ` 事务导出表数据的范围 (通过 `SET TRANSACTION SNAPSHOT` 在同一个快照下工作)。参照：[Day 8: How to speed up pg_dump](#/docs/8)。
