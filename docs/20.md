...
> 原作者：Nikolay Samokhvalov · [英文原文](https://gitlab.com/postgres-ai/docs/-/blob/80536bb97f28d1007ad09627db434c2b7016da1f/docs/postgres-howtos/database-administration/backup-recovery/how-to-use-pg-restore.md) · 中文翻译与校订：PostgreSQL学徒
...
当处理以"目录"格式 (`-Fd`) 创建的转储文件时，你可以使用 `-j` (`--jobs`) 选项，通过运行多个 `pg_restore` 工作进程来加速恢复过程。但是，如果有一个或几个大表，这种并行化并不会有帮助 — 单表的转储或恢复不支持并行化。在第 8 天的 [pg_dump](#/docs/8) 指南中，我们讨论过这个问题。
...
- 指定 `--exit-on-error` 时，恢复会在遇到错误后停止。处理原因后，可通过 `pg_restore -l` 导出 TOC（目录清单），再用 `-L` 指定调整后的清单，恢复尚未完成的对象。
- 未指定 `--exit-on-error` 时，恢复会继续执行后续命令。结束后仍需检查错误，并根据对象依赖关系，补做失败对象及受影响的依赖对象。
- 不论采用哪种方式，都应仔细核对 TOC 中的依赖顺序，避免只重试最后一个报错对象，却遗漏它依赖的表、数据或约束。

> 译者校订：以上补做方式需要结合实际恢复状态判断。如果使用 `--single-transaction`，失败后整个事务会回滚，不能按“前面的对象已成功恢复”处理。
