...
> 原作者：Nikolay Samokhvalov · [英文原文](https://gitlab.com/postgres-ai/docs/-/blob/80536bb97f28d1007ad09627db434c2b7016da1f/docs/postgres-howtos/schema-design/ddl-operations/how-to-add-a-column.md) · 中文翻译与校订：PostgreSQL学徒
...
对于适用快速添加列优化的默认值，PostgreSQL 不会立即重写所有现有行，而是在读取旧行时补出缺失的值。默认值表达式保存在 [pg_attrdef](https://postgresql.org/docs/current/catalog-pg-attrdef.html) 中，用于旧行的缺失值则记录在 [pg_attribute.attmissingval](https://www.postgresql.org/docs/current/catalog-pg-attribute.html) 中。示例：
...
- 对于批处理，先选择待更新行的范围。上游这里只给出了条件片段，下面将其写成可执行的选行查询；`:batch_size` 是需要在 psql 中预先设置的变量：
  ...
  > 译者校订：PostgreSQL 不支持直接在 `UPDATE` 后追加 `ORDER BY ... LIMIT`。实际回填时，应通过子查询或 CTE 选取这一批主键，再据此更新，并按业务规则计算新列的值。
