...
> 原作者：Nikolay Samokhvalov · [英文原文](https://gitlab.com/postgres-ai/postgresql-consulting/postgres-howtos/-/blob/41b389c5b820ab8e31255728c8b768774f4ff1bb/0073_how_to_analyze_heavyweight_locks_part_3_persistent_monitoring.md) · 中文翻译与校订：PostgreSQL学徒

如果活跃会话 (`pg_stat_activity`，`state='active'`) 的峰值伴随着独占锁 (`pg_locks`，`mode ~* 'exclusive'`) 的峰值发生，我们需要分析被阻塞和阻塞的会话，以了解根本原因。

之前讨论过临时的分析方法：

- [Day 22, How to analyze heavyweight locks, part 1](#/docs/22)
- [Day 42, How to analyze heavyweight locks, part 2: Lock trees (a.k.a. "lock queues", "wait queues", "blocking chains")](#/docs/42)
- 特别是对于 DDL 被阻塞的情况： [Day 71, How to understand what's blocking DDL](#/docs/71)
